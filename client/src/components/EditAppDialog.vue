...
// 监听传入的应用数据变化
watch(() => props.appData, (newAppData) => {
  if (newAppData && Object.keys(newAppData).length > 0) {
    formData.id = newAppData.id
    formData.title = newAppData.title || ''
    formData.icon = newAppData.icon || ''
    formData.url = newAppData.url || ''
    formData.packageName = newAppData.packageName || ''
    formData.remark = newAppData.remark || ''
    formData.accounts = newAppData.accounts && newAppData.accounts.length > 0 
      ? JSON.parse(JSON.stringify(newAppData.accounts)) 
      : [{ username: '', password: '' }]
  }
}, { immediate: true })
